...
class TranslationConfig:
    random_seed:int=42;dataset_name:str="para_crawl";language_pairs:Tuple[Tuple[str,str],...]=(("en","fr"),("en","de"),("en","es"));model_checkpoint:str="google/mt5-xl";output_dir:Path=field(default_factory=lambda:Path("output"));num_proc:int=4;batch_size:int=32;learning_rate:float=2e-5;num_train_epochs:int=5;max_input_length:int=512;max_target_length:int=512;hf_repo_id:str=os.getenv("HF_REPO_ID");hf_token:str=os.getenv("HF_TOKEN");use_wandb:bool=True;wandb_project:str="multilingual-translation";use_fp16:bool=True;gradient_accumulation_steps:int=2;warmup_steps:int=500;eval_steps:int=1000;save_steps:int=1000;max_grad_norm:float=1.0;use_8bit_quantization:bool=False;use_peft:bool=True;lora_r:int=16;lora_alpha:int=32;lora_dropout:float=0.05;use_accelerate:bool=True;use_deepspeed:bool=False;deepspeed_config_path:Optional[str]="ds_config.json";use_ray:bool=False;num_gpu:int=1;use_multi_gpu:bool=False;log_level:str="INFO";early_stopping_patience:int=3;scheduler_type:str="linear";weight_decay:float=0.01;adam_epsilon:float=1e-8;max_grad_norm:float=1.0;clipping_threshold:float=1.0;use_fsdp:bool=False
...
def load_datasets(self)->DatasetDict:
    ...
    for src_lang,tgt_lang in self.config.language_pairs:
        ...
        except Exception as e:raise DatasetLoadError(f"Error loading dataset for {lang_pair}: {str(e)}")
...
def run_inference(self,texts:List[str],src_lang:str,tgt_lang:str)->List[str]:
...
def translate_batch(self,texts:List[str],src_lang:str,tgt_lang:str)->List[str]:
    return self.translation_model.run_inference(texts,src_lang,tgt_lang)
